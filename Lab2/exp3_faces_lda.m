...
for r = 1:nReplications
    ...
    nbMaxFeatures = size(Weight_lda, 2);
    ...
    for nbFeatures = 1:nbMaxFeatures
        ...
        error = prcrossval(training_dataset_lda(mixmat, 1:nbFeatures), classifier, nFolds);
    ...
    for k = 1:10
        classifier =  knnc([], k); 
        error = prcrossval(training_dataset_lda(:, 1:bestNbFeatures), classifier, nFolds);
        logError_k(k) = error;
    end
    tTrain = toc;
    disp(['Trainig time: ' num2str(tTrain) 's']);       
    ...
    disp(['Validation error: ' num2str(bestError_k*100) '%']);
    tic;
    classifier =  knnc(training_dataset_lda, bestK); 
    errClassifier(r) = testc(test_dataset_lda, classifier);
    tTest = toc;
    disp(['Test time: ' num2str(tTest) 's']);        
    disp(['Test Err: ' num2str(errClassifier(r)*100) '%']);
    disp('Confusion Matrix: ');
    confMat{r} = confmat(getlabels(test_dataset_lda), labeld(test_dataset_lda * classifier))
    
    listMisclassified = [getlabels(test_dataset_lda), labeld(test_dataset_lda * classifier)];
    ...
    for idxImage = 1:size(listMisclassified, 1)
        ...
        imwrite(image, sprintf(['knn_lda_' num2str(r) '_' num2str(listMisclassified(idxImage, 1)) '_' num2str(listMisclassified(idxImage, 2)) '.png']))
    end
end
save('matconf_knn_lda', 'confMat');
